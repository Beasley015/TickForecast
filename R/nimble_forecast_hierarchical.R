...
model.code <- nimbleCode({
  # hyperpriors for mice and weather submodels
  
  for(site in 1:nsite){
	  ### priors: phi = survival, theta=transition
    ...
    for (j in 1:n.beta) { # the hell is n.beta?--------
		...
		for (i in 1:ns) {
			sig[i] ~ dinvgamma(pr.sig[i, 1], pr.sig[i, 2]) #the hell is sig? -------
		}
