  ...
  for(site in 1:nsite){
    ...
    for (j in 1:n.beta) {
      ...
      # This will be hierarchical after hierarchical intercepts work
		...
		for (i in 1:ns) { # Check what 'ns' is -----------
      ...
      if (miceAndWeather) {
			  ...
			  logit(l2n[t, site]) <- theta.ln[site] + beta[1] * mice[t, site]
			  logit(n2a[t, site]) <- theta.na[site] + beta[2] * mice[t, site]
		  } else {
			  logit(l2n[t, site]) <- theta.ln[site]
			  logit(n2a[t, site]) <- theta.na[site]
		  }
	    
		  cgdd[t, site] ~ dnorm(gdd[t, site], tau = tau.cgdd)
		  ...
		  for (p in 1:n.plots) {
			  ...
			  y[1, t, p, site] ~ dpois(dx[1, t, p, site]) # should this be y[2,,,]?-----
	  ...
	  for (t in 2:horizon) {
		  ...
		  Ex[1:ns, t, site] <- A[1:ns, 1:ns, t - 1, site] %*% x[1:ns, t - 1, site]
