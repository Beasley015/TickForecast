...
model.code <- nimbleCode({
  ...
  theta.l2n.tau ~ dgamma(pr.l2n.tau[1], pr.l2n.tau[2])
  theta.n2a.tau ~ dgamma(pr.l2n.tau[1], pr.l2n.tau[2])
  ...
  for(site in 1:length(sub.sites)){
	  ### priors
    phi.l.mu[site] ~ dnorm(phi.l.mean, tau = phi.l.tau)
    phi.n.mu[site] ~ dnorm(phi.n.mean, tau = phi.n.tau)
    phi.a.mu[site] ~ dnorm(phi.a.mean, tau = phi.a.tau)

    theta.ln[site] ~ dnorm(theta.l2n.mean, tau=theta.l2n.tau)
    theta.na[site] ~ dnorm(theta.n2a.mean, tau=theta.n2a.tau)

    for(j in 1:n.beta){
      beta[j,site] ~ dnorm(beta.mean[j], tau = beta.tau[j])
    }
