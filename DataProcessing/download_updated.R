...
DAAC_Set_Credential <- function(replace = FALSE, folder.path = NULL) {
  if (replace) {
    message("Replace previous stored NASA DAAC credentials.")
  }
...
from <- "2016-01-01"
to <- "2025-12-31"
dois <- c("10.5067/HLS/HLSS30.002", "10.5067/HLS/HLSL30.002")
outdir <- "/projectnb/dietzelab/neochatt/MSLP/HLS/TREE"
# SWIR - Landsat (B06&07), Sentinel (B11&12)
band <- c('B02','B03','B04','B8A','B11','B12','B05','B06','B07', 'Fmask')
credential.folder <- "/usr4/ugrad/neochatt/cred" # Replace with path to user's own credential folder

for (doi in dois) {
  message("Downloading for DOI: ", doi)
  paths <- NASA_DAAC_download(ul_lat = ul_lat, 
                              ul_lon = ul_lon, 
                              lr_lat = lr_lat, 
                              lr_lon = lr_lon,
